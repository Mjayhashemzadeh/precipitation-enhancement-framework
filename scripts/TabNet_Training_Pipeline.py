...
class PrecipitationModelingPipeline:
    ...
    def train_and_evaluate(self, X, y, stations):
        ...
        self.log(f"🚀 Training TabNet with parameters: {tabnet_params}")
        ...
        for i, (train_idx, test_idx) in enumerate(kf.split(X)):
            ...
            y_train = y.iloc[train_idx].values.reshape(-1, 1).astype(np.float32)
            X_test = X.iloc[test_idx].values.astype(np.float32)
            y_test = y.iloc[test_idx].values
            
            model.fit(
                X_train, y_train,
                eval_set=[(X_test, y_test.reshape(-1, 1).astype(np.float32))],
                **fit_params
            )
            
            y_pred = model.predict(X_test).flatten()
            fold_metrics = self.evaluate_model(y_test, y_pred)
        ...
        X_final = X.values.astype(np.float32)
        y_final = y.values.reshape(-1, 1).astype(np.float32)
        
        final_model.fit(X_final, y_final, **fit_params)
        ...
        plt.title('Feature Importance (Final TabNet Model)')
        plt.tight_layout()
    ...
    def save_results(self, model, feature_importance, kfold_df, logo_df, station_df):
        ...
        model_path = os.path.join(output_dir, 'tabnet_model.zip')
    ...
    def run_pipeline(self, remote_source='ERA5'):
        self.log(f"🚀 Starting precipitation modeling pipeline with {remote_source} data using TabNet")
